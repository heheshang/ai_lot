...
pub async fn emergency_stop(
    db: State<'_, Database>,
    user_id: String,
) -> Result<EmergencyStopReport, String> {
    log::error!("EMERGENCY STOP triggered by user: {}", user_id);

    // Get services from database state
    let strategy_engine = db.get_strategy_engine();
    let exchange = db.get_exchange();
    let db_wrapper = db.as_trade_db();
    ...
    match emergency_service.emergency_stop_all(&user_id).await {
        Ok(report) => {
            log::error!("EMERGENCY STOP completed successfully: {:?}", report);
            Ok(report.into())
        }
        Err(e) => {
            log::error!("EMERGENCY STOP failed: {}", e);
            Err(format!("Emergency stop failed: {}", e))
        }
    }
}
